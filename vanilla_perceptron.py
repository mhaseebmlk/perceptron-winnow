...
class VanillaPerceptron:
	...
	def perceptron_test(self,img):
		acts=[np.dot(self.perceptrons[digit],img) for digit in range(len(self.perceptrons))]
		predicted_digit=acts.index(max(acts))
		return predicted_digit

def print_usage():
	print('Usage:')
	print('python vanilla_perceptron.py [size of training set] [no of epochs] [learning rate] [path to DATA_FOLDER]')
...
def main():
	...
	# for r in rate_tuning_params[2]:
	# 	training_set_sz=rate_tuning_params[0]
	# 	num_epochs=rate_tuning_params[1]
	# 	learning_rate=r

	vanilla_perceptron=VanillaPerceptron(training_parsed,training_set_sz,num_epochs,learning_rate)
	...
	for img, lbl in training_set:
		predicted_digit=vanilla_perceptron.perceptron_test(img)
	...
	test_accuracy=(test_accuracy/len(testing_parsed)) * 100
		...
		# print '{},{},{},{},{}'.format(r,test_accuracy,test_f1,training_accuracy,training_f1)
